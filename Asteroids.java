...
//		Map<KeyCode, Boolean> pressedKeys = new HashMap<>(); // keys are mapped to values, 
		...
		new AnimationTimer() {
			...
			public void handle(long nanotime) {
					...
					if (ship.collide(asteroid) && !ship.isInvincible) {
						...
						if (ship.lives <= 0) {
							...
							if (result.isPresent()) {
							    ...
							    highScores.saveNameAndScore(name, text);
				...
				if (pressedKeys.getOrDefault(KeyCode.SPACE, false) && canShoot && System.nanoTime()-shootTimer >= 2e8 && bullets.size() < 50) {
