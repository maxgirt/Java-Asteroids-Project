...
public class Asteroids  {
    ...
    //store scores and rank them
    private HighScores highScores;
    private Stage menuStage;

    private void resetShip(Ship ship) {
        ship.lives--; // ship life - 1
    ...
    int currentLevel = 1;
    int maximumLevel = 5;
    //当前操作对象 第一个参数为x坐标  第二个参数为y坐标
    private Ship ship = new Ship(150,100);

    private List<Asteroid> asteroids = new ArrayList<>();

    public void startGame(Stage stage, Menu menu) throws Exception {  //开始游戏
        Random rnd = new Random(); //获取随机变量
        Pane pane = new Pane(); //设置
        pane.setPrefSize(paneWidth, paneHeight); //设置

        Scene scene = new Scene(pane);//设置
        stage.setTitle("Asteroids");
        Text text = new Text(10, 20, "Points: 0"); //显示分数
...
//        Asteroid asteroid = new Asteroid(rnd.nextInt(paneWidth), rnd.nextInt(paneHeight), 50, 10);
         //第一个参数为陨石的x坐标  第二个参数为陨石的y坐标  第三个参数为大小  第四个参数为速度
     Asteroid asteroid = new Asteroid(rnd.nextInt(paneWidth), rnd.nextInt(paneHeight), 50, 10);
...
//        AtomicInteger points = new AtomicInteger();//原子操作
        AtomicInteger points = new AtomicInteger();//原子操作
        List<Bullet> bullets = new ArrayList<>();//自己发射的子弹
        ...
        List<Bullet> alienBullets = new ArrayList<>();//外星人发射的子弹
        List<Bullet> deadAlienBullets = new ArrayList<>();//子弹
        ...
        List<Alien> aliens = new ArrayList<>(); //存储外星人的个数
        ...
        new AnimationTimer() {
            ...
            public void handle(long nanotime) {
                ...
                //up and down, if speed less than 10 speed up
                ...
                aliens.forEach(a -> {
                    ...
                    if (ship.collide(a) && !ship.isInvincible) {
                        ...
                        if (ship.lives <= 0) {
                            Integer socre1=points.get();
                            System.out.println("分数为:"+socre1);
                            try {
                                menu.getHighScores().saveScore( socre1);
                            } catch (IOException e) {
                            ...
                            ButtonType buttonType=new   ButtonType("Restart",ButtonType.OK.getButtonData());
                            DialogPane dialogPane=new DialogPane();
                            dialogPane.setContentText("Do you want to restart?");
                            dialogPane.getButtonTypes().add(buttonType);
                            Button Ok=(Button) dialogPane.lookupButton(buttonType);
                            Stage dialogStage = new Stage();
                            Scene dialogScene = new Scene(dialogPane);
                            dialogStage.setScene(dialogScene);
                            dialogStage.setTitle("退出!");
                            dialogStage.show();
                            Ok.setOnAction(event->{
                                ...
                                alienBullets.clear();
                                deadAlienBullets.clear();
                                aliens.clear();
                                asteroids.clear();


                                //Update the following node information
                                start();
                                Ship ship1 = new Ship(150, 100);
                                ship = ship1;
                                asteroid.resetAsteroid(rnd.nextInt(paneWidth), rnd.nextInt(paneHeight), 50, 10);
                                asteroids.add(asteroid);
                                livesText.setText("Lives: " + ship.lives);
                    ...
                    if (ship.collide(bullet) && !ship.isInvincible) {
                        ...
                        if (ship.lives <= 0) {
                            ...
                            Ok.setOnAction(event->{
                                ...
                                //clear the bullet remain in window
                                pane.getChildren().clear();
                                bullets.clear();
                                deadBullets.clear();
                                deadAsteroids.clear();
