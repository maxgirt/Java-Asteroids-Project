...
public class Asteroids  {
	...
	public static int paneHeight = 800;
	public static int paneWidth = 1200;
    int currentLevel = 1;
    int maximumLevel = 5;
	...
	public void startGame(Stage menuStage, Stage stage, Menu menu) throws Exception {
        ...
        Text text = new Text(10, 20, "Points: " + score);
        text.setFont(font);
        text.setFill(Color.WHITESMOKE);
        text.setX(paneWidth * 0.16 );
        text.setY(paneHeight * 0.04 );
...
//		Map<KeyCode, Boolean> pressedKeys = new HashMap<>(); // keys are mapped to values, 
        ...
        livesText.setX(paneWidth * 0.05 );
        livesText.setY(paneHeight * 0.04 );
        pane.getChildren().add(livesText);

        Text jumpText = new Text("Jump: ");
        jumpText.setFont(font);
        jumpText.setFill(Color.WHITESMOKE);
        jumpText.setX(paneWidth * 0.30 );
        jumpText.setY(paneHeight * 0.04 );
		...
		new AnimationTimer() {
			...
			public void handle(long nanotime) {
			    new Thread(() -> {
			        sfx1.playMainTheme();
			    }).start();				long elapsedTime = nanotime - lastUpdateTime;
				...
				for (Bullet bullet : alienBullets) {
					if (bullet.getDistance() > 15) { // 如果子弹已经存在超过 2 秒
				...
				asteroids.forEach(asteroid -> {
                    ...
                    if (ship.collide(asteroid) && asteroid.getSize() == 50) {
                    ...
                    } else if (ship.collide(asteroid) && asteroid.getSize() == 30) {
                        Asteroid newAsteroid1 = new Asteroid(asteroid.getCharacter().getTranslateX(),
					...
					bullets.forEach(bullet -> {
					    if (bullet.collide(asteroid) && asteroid.getSize() == 50) {
	                        ...
	                        text.setText("Points: " + score); // update the score display
					        Asteroid newAsteroid1 = new Asteroid(asteroid.getCharacter().getTranslateX(),
					        asteroid.getCharacter().getTranslateY(),30, 35);
					        newAsteroid1.getCharacter().setStroke(Color.WHITESMOKE);
					        mediumAsteroids.add(newAsteroid1);
			...
			// logic to increase level 
				...
				if (ship.jumps >=1) {
                    ...
                    jumpIsOn.setLayoutX(paneWidth*0.36);
                    jumpIsOn.setLayoutY(paneHeight*0.04);
                    ...
                    jumpIsOff.setLayoutX(paneWidth*0.36);
                    jumpIsOff.setLayoutY(paneHeight*0.04);
	...
	public void levelScreen(Random rnd, Pane pane) {
        ...
        levelText.setLayoutX(paneWidth/2 - 42);
        levelText.setLayoutY(paneHeight/2 - 75);
        pane.getChildren().add(levelText);		
		
	  // Set the button's position after it is added to the scene graph
		continueButton.setLayoutX(paneWidth/2 - 60);
		continueButton.setLayoutY(paneHeight/2 - 60);
